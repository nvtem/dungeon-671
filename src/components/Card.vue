<template>
    <div
        @click="$emit('click')"
        :class="['card', {'card--player': type === 'player', 'card--hover': isHovered }]"
        @mouseenter="() => {if (isNearPlayerCard()) isHovered = true}"
        @mouseleave="() => {if (isHovered) isHovered = false}"
    >
        <div class="hp" v-if="showHp">
            <img class="hp-icon" src="../assets/icon-hp.png"><div class="hp-value">{{hp}}</div>
        </div>
        ...
        <div class="poison-countdown" v-if="isPoisoned">
            <img class="poison-countdown-icon" src="../assets/icon-poison-countdown.png"><div class="poison-countdown-value">{{poisonCountdown}}</div>
        </div>
    ...
    export default {
        ...
        computed: {
            ...
            imageSrc() {
                ...
                return require("../assets/" + this.type + suffix + ".png")
            },

            powerIconSrc() {
                return ('hasPoisonedWeapon' in this && this.hasPoisonedWeapon) ? require('../assets/icon-power-poison.png') : require('../assets/icon-power.png')
            },
